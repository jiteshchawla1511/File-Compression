...
void huffman::traverseArr(Node* curr,string str){
	if(curr->left == NULL && curr->right == NULL){
		curr->code = str;
		return;
	}
	traverseArr(curr->left,str + '0');
	traverseArr(curr->right,str + '1');
}
...
string huffman::decToBin(int value){
	string curr = "";
	while(value > 0){
		if(value % 2 == 0){
			curr.push_back('0');
